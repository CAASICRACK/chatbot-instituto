// api/chat.js
// Desplegar en Vercel. Requiere la variable de entorno DEEPSEEK_API_KEY.

export const config = {
  runtime: "edge", // opcional, pero recomendado para menor latencia
};
...
5. UBICACIÓN Y CONTACTO:
   ...
   - Horario de clases: diurno y vespertino (Anatomía Patológica en turno de tarde).
...
- Si te preguntan algo que no está en esta información, responde: "Lo siento, no dispongo de esa información en este momento. Te recomiendo contactar directamente con el centro en el 927 01 60 80 o en ies.albalat@edu.gobex.es."
...
export default async function handler(req) {
  ...
  try {
    ...
    if (!message) {
      return new Response(JSON.stringify({ error: 'Falta el campo "message"' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    if (!deepseekResponse.ok) {
      ...
      return new Response(
        JSON.stringify({ error: 'Error al comunicarse con el asistente.' }),
    ...
    return new Response(JSON.stringify({ response: reply }), {
      ...
      headers: {
        'Content-Type': 'application/json',
        'Access-Control-Allow-Origin': '*', // Permite solicitudes desde cualquier origen (ajusta si es necesario)
      },
    });
  } catch (error) {
